...
Participants made this a hard test of "AI helps novices most": more than a year of Python, weekly use, prior AI experience, never Trio ([Fig 17](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/participant_distribution.svg)). A warm-up task, then 35 minutes for two Trio tasks: a concurrent timer and a record-retrieval function with error handling. Then a 14-question, 27-point quiz over 7 Trio concepts in three skill types — conceptual understanding, code reading, debugging ([Fig 20](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/question-types.svg)); code-writing was excluded, since syntax is what AI fixes cheapest.
...
The treatment assistant was no toy: GPT-4o with access to the participant's code, able to produce the full correct solution to both tasks. Four pilot studies tuned the design — the first platform had 35% non-compliance (controls using AI anyway); a later pilot ([Fig 5](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/pilot_study_D.svg)) showed the effect so strongly (d=1.7) that the pre-registration assumed a conservative d=0.85 for power. Integrity controls included pledges and screen recordings of every participant ([Figs 21–24](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/img/SC-Pledge-NoAI.png); [Figs 25–27](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/img/SC-Task-No-AI.png) show both platforms).

![thumb:Fig 17 - Participant distribution](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/participant_distribution.svg)

## Results: the 17% toll

The main results ([Fig 6](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/main_study.svg)): a 4.15-point gap on 27 points — 17%, two grade points — with no significant difference in completion time; the effect survives controlling for warm-up speed (d=0.725, p=0.016). [Fig 7](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/main_study_yoe.svg) shows the control group scoring higher at every experience level, and [Fig 8](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/quiz_score_subarea.svg) shows where the damage concentrates: the largest gap is in **debugging**, the smallest in code reading. Both groups read code; only the control group had to fix it.

![thumb:Fig 6 - Main study: task time and quiz score (n=52)](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/main_study.svg)
![thumb:Fig 8 - Score breakdown by question type](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/quiz_score_subarea.svg)
...
The qualitative analysis explains the missing speedup: heterogeneity. Watching every screen recording, the researchers found six AI-interaction personas ([Fig 11](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/Personas-Simple.svg)), with completion times and quiz scores that barely overlap:
...
![thumb:Fig 11 - The 6 AI interaction personas with outcomes](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/Personas-Simple.svg)

The gap between worst and best usage is the real story: 24–39% vs 65–86% on the same quiz with the same assistant. The difference is not whether AI was used; it is what happened after the AI answered. The time side confirms it: some participants spent up to 11 minutes composing queries and asked 15 questions ([Fig 12](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/ai_interaction_time.svg)); debugging-heavy query mixes correlated with slower times and lower scores ([Fig 18](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/ai_query_Completion_Time.svg), [Fig 19](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/ai_query_Quiz_Score.svg)). And the paste-vs-type analysis ([Fig 13](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/paste_behavior.svg)) has a twist: pasting AI output was fastest, manually copying it was as slow as control — but the two scored the same on the quiz. *Spending time typing doesn't build understanding.*

![thumb:Fig 13 - Pasting vs manual copying of AI output](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/paste_behavior.svg)

## Errors are the curriculum

The mechanism the paper lands on is the most old-school finding in it: **errors teach.** The median control participant hit three errors; the median AI participant hit one ([Fig 15](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/error_types_by_condition.svg)). Of the twelve error-free completers, only two were control. The errors that matter are the Trio-specific ones — `RuntimeWarning` (a coroutine never awaited), `TypeError` (a coroutine passed where an async function was expected) — because they force exactly the conceptual knowledge the quiz tests ([Fig 14](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/error_types.svg)). The control group did not learn *despite* the errors; the errors were the lesson plan. Active coding time tells the same story from the other side ([Fig 16](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/active_time_vs_quiz_score.svg)): AI shifted time from writing to reading AI output. The AI group's own feedback confirms it — they felt "lazy" with "gaps in (their) understanding."

![thumb:Fig 15 - Errors by condition: control meets the concepts](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/error_types_by_condition.svg)
...
![thumb:Fig 1 - Overview of results: skills down, time flat](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/Hero-Results-Personas.svg)
![thumb:Fig 2 - Motivation: novices and AI in the workplace](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/ai-skills.svg)
![thumb:Fig 5 - Pilot Study D: time and quiz](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/pilot_study_D.svg)
![thumb:Fig 7 - Task time and quiz by years of experience](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/main_study_yoe.svg)
![thumb:Fig 9 - Self-reported enjoyment and learning](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/task_learning_enjoyment.svg)
![thumb:Fig 10 - Self-reported task difficulty](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/task_difficulty.svg)
![thumb:Fig 12 - AI interaction time and query count](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/ai_interaction_time.svg)
![thumb:Fig 14 - All errors by type](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/error_types.svg)
![thumb:Fig 16 - Active coding time vs quiz score](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/active_time_vs_quiz_score.svg)
![thumb:Fig 18 - Queries vs completion time](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/ai_query_Completion_Time.svg)
![thumb:Fig 19 - Queries vs quiz score](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/ai_query_Quiz_Score.svg)
![thumb:Fig 20 - Example evaluation question types](https://ar5iv.labs.arxiv.org/html/2601.20245/assets/question-types.svg)
...
- [Summary infographic of the study (LinkedIn, August 2026)](https://blog.hackspree.com/images/skill-formation-linkedin-summary.jpg) — embedded at the top of this post.
