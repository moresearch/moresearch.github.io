...
- The PDF viewer on /talks/NNN/ must be visually dominant within the second (right) column. The viewer container must present a single PDF page (page 1) sized to the page aspect ratio (210/297) and scaled to fit. Implement this with `.pdf-container` using `aspect-ratio: 210/297` and `max-height: 72vh`. The embedded viewer (iframe/object) must fill the container (`width:100%; height:100%`) and have scrolling disabled or overflow hidden so only one page is visible at a time. The viewer must never overflow or stretch beyond its column on any screen size. All non-PDF content (navigation, fallback, etc.) must use a smaller font and minimal spacing.
