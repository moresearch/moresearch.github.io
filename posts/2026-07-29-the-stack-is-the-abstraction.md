...
> The stack is not a collection of tools. It is a set of constraints on the design space. What remains is small enough to search — small enough that the designer's intent, Brooks' vision in the mind's eye, can reach the code intact.
...
Beyond search, the stack makes intent manifest. Dijkstra argued that the intellectual manageability of software depends on levels of abstraction — each level a self-contained world. Brooks called the result *conceptual integrity* — one design voice, every part consistent. He described the design as a shared invisible entity, existing in the mind's eye before a line of code is written. The stack is what carries it from mind to machine. The designer thinks at the level of the problem. The stack translates. When the vocabulary is consistent, the design is coherent. When the design is coherent, the intent survives the implementation. Brooks' phrase: the designer must see the design before it is built. The stack is what lets them see it.
...
The stack constrains technology. Patterns constrain structure. Brooks gave three principles for achieving conceptual integrity, all negative, all constraints: Propriety (do not introduce what is immaterial), Orthogonality (do not link what is independent), Generality (do not restrict what is inherent). Buschmann et al. extended this to the architectural level in *Pattern-Oriented Software Architecture* (1996): architectural patterns are constraints on component arrangement. Layers says dependencies flow down. MVC says model, view, controller — separate them. Pipes and Filters says each step transforms data independently. Each pattern eliminates a region of the structural design space. You do not decide to use Layers and then decide how to organize dependencies. Layers *is* the decision. The pattern made it before you arrived.
...
> The stack is not a productivity tool. It is a search constraint. It ensures the agent's search converges on the designer's intent — the conceptual integrity Brooks described — regardless of the path taken. Today, and tomorrow, when a different agent with a different model searches the same codebase.

Spolsky's [Development Abstraction Layer](https://www.joelonsoftware.com/2006/04/11/the-development-abstraction-layer-2/) argued management insulates programmers from everything not code. The stack insulates the designer's intent from everything not the problem — including the agent that searches it. Brooks called that intent the design in the mind's eye. Spolsky called the organization that protects it the Development Abstraction Layer. Both succeed when invisible. The designer's job is to make them disappear.
