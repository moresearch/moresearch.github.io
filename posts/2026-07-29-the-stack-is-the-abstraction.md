...
Beyond search, the stack makes intent manifest. Dijkstra argued that the intellectual manageability of software depends on levels of abstraction — each level a self-contained world. Brooks called this *conceptual integrity* — one design voice, every part consistent. The stack provides both: the levels and the voice. The designer thinks at the level of the problem. The stack translates. When the vocabulary is consistent, the design is coherent. When the design is coherent, the intent survives.
...
The stack solves this by constraining not the path, but the region. Inside Rails, the agent still searches non-deterministically. Different variable names. Different query ordering. Different middleware arrangement. What it cannot do is choose Express. The region boundary is the stack. The search is free within it. Every path stays within the Rails region. Every path leads to the same architecture. The stack does not make the agent deterministic — nothing can. It makes the agent's non-determinism *harmless*.
