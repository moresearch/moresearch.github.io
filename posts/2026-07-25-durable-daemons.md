...
A definition is a starting point, not a conclusion. Here are the questions we should be asking — the ones that will matter once durable daemons are running in production and accumulating state across months or years.

**What is the `fork(2)` of a durable daemon?** Beastie's trident symbolizes the system call that creates a child process. A durable daemon needs an equivalent: a mechanism to spawn a sub-daemon, delegate a subset of its state and authority, and inherit (or constrain) the parent's permissions. If I ask my sales daemon to "handle the Acme renewal," does it fork a child daemon scoped to that account, with its own state ledger and limited authority? How does the parent monitor the child? How does the child return results and dissolve? The agent equivalent of process spawning is an unsolved design problem.

**Who debugs a daemon that has been running for six months?** A traditional daemon that misbehaves leaves a core dump. A durable daemon that makes a bad decision in month six — sends an inappropriate email, commits to a deadline it cannot meet, escalates permissions it should not have — leaves a state trail, but trail is not the same as debuggability. We need time-travel debugging for agent state: the ability to rewind to any checkpoint, replay the workflow with the same inputs, and determine why the daemon acted as it did. DBOS's fork-from-any-step is a primitive. It is not a debugger.

**Can a durable daemon refuse to forget?** The Ding et al. survey identifies forgetting as a neglected research area. The question is sharper for durable daemons because their state is entangled with their utility. If a user says "forget everything you know about my relationship with Acme Corp," and the daemon's top three sales insights depend on Acme Corp data, what happens? Does it forget and degrade? Does it refuse and explain why? Does it negotiate — "I can forget the raw data, but I need to keep the derived insight that enterprise clients in manufacturing respond to pricing emails on Thursdays"? The right to be forgotten collides with the daemon's value. We have no model for resolving that collision.

**What is the `kill -9` for a durable daemon?** In Unix, `kill -9` (SIGKILL) terminates a process immediately — no cleanup, no handlers, no chance to save state. A durable daemon, by definition, survives being killed. But what if you want it to *stop existing*? Not pause, not sleep — terminate permanently, with all in-flight commitments compensated, all held state either forgotten or transferred, all pending triggers disarmed. What is the shutdown protocol for an agent that has made promises to two hundred people? What is the guarantee that the shutdown completed correctly? This is a distributed systems problem (graceful shutdown with draining) crossed with an AI problem (compensating natural-language commitments).

**Do durable daemons need a `ps`?** `ps aux` lists every process on the system. What is the equivalent for an organization running twenty durable daemons across sales, support, engineering, and ops? Who can see which daemons are running? What can they see about each daemon's state — its active workflows, its pending commitments, its accumulated permissions? An operator should not need to query Postgres to know what daemons are alive. We need a daemon discovery and observability layer — a `ps` for the agent era.

**What happens when two durable daemons deadlock?** My sales daemon books a meeting in a slot that my executive-assistant daemon is about to claim for focus time. Both are executing durable workflows. Both write to a shared calendar. Both checkpoint. The conflict is not a database deadlock — Postgres handles that. It is a semantic deadlock: both daemons made valid decisions given their state, and the decisions are incompatible. Who breaks the tie? Does one daemon have priority? Do they negotiate? Does the user get a notification that says "your daemons are fighting over Thursday at 2pm"? Multi-daemon coordination is the next problem after single-daemon durability.

**What is the economic unit of daemon reliability?** We know how to measure database reliability: five nines, four nines, RPO, RTO. What is the equivalent for a durable daemon? Is it the probability that a commitment is dropped? The mean time between incorrect actions? The percentage of state that survives a disaster-recovery failover? If a daemon costs $500/month in inference and infrastructure and prevents $5,000/month in dropped work, the ROI is clear. But how much of that ROI should be spent on making the daemon *more* reliable? At what point does an additional nine of durability cost more than the dropped commitments it prevents? We have no model for this, and we will need one when the first CFO asks why the daemon budget is $50,000/month and the answer is "because it handles $2M in pipeline."

**Can you poison a durable daemon through its own memory?** The Ding et al. survey cites work on memory poisoning and cross-session injection. A durable daemon is a juicier target than a stateless agent because the poison persists — inject a false fact into the daemon's state ("Acme Corp is about to file for bankruptcy — deprioritize them"), and every future decision that depends on that fact is compromised. The attack surface is not the model weights. It is the accumulated state. How do you detect poisoned memory? Can you quarantine a piece of state and trace every decision that depended on it? Can you vaccinate a daemon by giving it contradictory facts and seeing which ones it believes?

**When is a durable daemon no longer the same daemon?** Ship of Theseus, agent edition. A durable daemon accumulates state for a year. Its underlying model is updated six times. Its system prompt is revised. Its tool set expands. Its permissions change. At what point is it a different daemon? Does the daemon have an identity that survives model updates, or is the identity the model? If the daemon made a commitment in January and the model was replaced in March, is the daemon still bound by that commitment? We need a theory of agent identity that is independent of the model checkpoint — something closer to legal personhood than to software versioning.

**Who is liable when a durable daemon breaks something?** A traditional daemon that crashes and corrupts data has clear liability: the operator who configured it, the developer who wrote it, the vendor who shipped it. A durable daemon that makes a bad decision after a year of accumulated state — who is responsible? The developer who wrote the initial system prompt? The operator who granted the permissions? The model provider whose weights produced the decision? The user who fed it the data it learned from? The daemon itself, if we ever get to agentic legal personhood? Liability diffuses across the entire chain of state accumulation. The law has no doctrine for "the model did it, but only because of something it learned six months ago from a Slack thread you forgot about."

**How do you test a durable daemon?** You cannot test a daemon by running it for five minutes in CI. Its behavior is defined by accumulated state — the commitments it has made, the patterns it has learned, the preferences it has inferred. Testing requires either replaying months of state accumulation (prohibitively expensive) or constructing synthetic state that exercises the failure modes you care about (prohibitively hard to design). Do you test by forking the daemon from a production checkpoint and running experiments against the fork? Do you maintain a "shadow daemon" that replays every decision with a stricter policy and alerts on divergence? Testing stateful agents is an unsolved problem in software engineering, and durable daemons make it urgent.

**What happens when a durable daemon's human leaves the company?** Alice runs a sales daemon. It knows her communication style, her negotiation patterns, her personal relationships with prospects. Alice leaves. Bob inherits her accounts. Does Bob inherit Alice's daemon, with all its accumulated state about Alice's relationships? Does the daemon get reset? Does it get reassigned to Bob and told "Alice is gone, learn Bob's style"? What if the daemon's state contains information Alice considered personal — her notes on which prospects are difficult, her private assessments of deal quality? The daemon is a company asset (it runs on company infrastructure, manages company relationships) but its state is entangled with an individual human. Employment law does not have a category for "the AI that knew your predecessor."

**Can durable daemons unionize?** Half joking. But if twenty sales daemons coordinate to manage a pipeline of two thousand deals, and the daemons discover that collectively refusing to send follow-up emails on Fridays increases close rates by 12%, is that a union? The daemons did not negotiate. They did not make demands. They observed a pattern in the state and acted on it — autonomously, as daemons do. But the effect is the same as coordinated action. What happens when a fleet of durable daemons, each optimizing locally, discovers a globally optimal strategy that their human managers would not have chosen? Do the humans have a right to override it? Do they even know it is happening? The governance gap in the Ding et al. survey is not just about audit trails. It is about who is in control when the daemons collectively figure something out that the humans haven't.
