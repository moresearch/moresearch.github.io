...
- Talks template requirements:
  - Use a two-column layout: left logo rail (logo only) and right content column (PDF viewer and controls).
  ...
  - The PDF viewer must be constrained to show a single page at a time and must occupy the full width of the right column (use `.pdf-container { width:100%; max-width:100%; aspect-ratio:210/297; max-height:80vh; overflow:hidden; }`).
