...
![FreeBSD — the operating system that kept the daemon tradition alive. Beastie, the BSD Daemon drawn by John Lasseter in 1988, remained the official project mascot even after this modern wordmark was introduced.](/images/freebsd-logo.png)
...
We love BSD over Linux for one reason that matters here: BSD is designed as a whole system — one source tree, one team, one coherent design — and its daemons are first-class citizens of that design, not afterthoughts assembled from separate projects by a distribution maintainer.
