...
The terms are a protocol, and engineers will recognize the shape. A lender advances capital sized against your recurring revenue — commonly several months of it. The company repays a fixed percentage of monthly revenue, typically 5–8% (lenders range from 2–10%). The obligation ends when cumulative repayments reach the advance plus a multiple — typically 1.5–2.5x — or when the term limit passes, usually three to five years, whichever comes first; in most structures the remaining balance is forgiven at the limit, so the cap is a ceiling, not a promise. No equity, no board seat, no fixed payment that can sink a company in a bad month. Strong months pay more; weak months pay less. Lenders may still file a lien on business assets, or ask for a personal guarantee at the small end — the telemetry is the underwriting collateral, not the only legal one.
...
Reported global volume surpassed $9.8 billion in 2025, with more than a hundred active lenders — Lighter Capital, Capchase, ClearCo, Pipe, Wayflyer, Founderpath, Recur Club — and nearly all of them underwrite through the same stack a modern product already runs: bank feeds, payment processors, accounting APIs.

## The arithmetic nobody does

The cap is the price, and duration sets the rate. A $1M advance at a 1.8x cap costs $800,000. Repaid over 24 months, that is roughly 40% a year on the original principal; over 48 months, roughly half that. RBF sellers quote the percentage of revenue, never the annualized cost, and the reason is structural: the rate depends on how fast revenue grows, which is the one number the lender cannot know — and the one number the roadmap determines.

Work the growth dependence through and the arithmetic cuts in a surprising direction. Repayment is a share of revenue, so the cap is hit fastest when revenue grows fastest. Make it concrete: $100k in monthly revenue, 6% repayment, a 1.8x cap. At 10% monthly growth, the cap is reached in about three years — an annualized cost near 25%. At 3% growth, it takes more than six years, at roughly half the annualized cost (and if the term limit arrives first, the balance is typically forgiven — cheaper still). Same loan, same cap, same company at closing; the only difference is the growth the team subsequently ships. **The rate was never in the contract; it was in the roadmap.**

Which means the instrument is cheapest, in rate terms, for the companies that need capital least — and the fastest-growing product is exactly the one where equity would have been cheapest relative to its trajectory. The equity comparison is a valuation question; the rate is a growth question — and growth is an engineering question.

## Where software engineering decides the terms

The RBF term sheet is written in your code, in four places.

**Gross margin is an architecture decision.** The lender's first question is what share of revenue survives the cost of delivering the product — and for an AI product, that number is set in the serving layer. Inference routing, model batching, caching, prompt compression, fallback to cheaper models: every optimization that cuts cost per request is, in financing terms, an improvement to the company's collateral. Every point of margin recovered is a point the repayment percentage cannot consume. Most teams report gross margin as a quarterly number; the teams that treat it as a per-feature budget get financed differently.
...
**The repayment schedule is your usage curve.** This is the line that makes the whole instrument click for engineers: the loan is serviced by your product's throughput. Every revenue-generating agent call, every paid seat, every metered event is a drip into the repayment — free-tier usage is not collateral. The financing and the product are the same loop. A product that grows paid usage grows its own repayment capacity — which is why RBF works for products with usage growth and fails for products with none.
