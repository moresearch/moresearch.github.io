...
- The PDF viewer on /talks/NNN/ must be visually dominant: maximize its size (width and height) within the second (right) column of the grid layout, and minimize the space taken by other elements. The viewer must never overflow or stretch beyond its column, on any screen size. The PDF viewer must only show a single page at a time (fit-to-page, no vertical scroll). All non-PDF content (navigation, fallback, etc.) must use a smaller font and minimal spacing.
