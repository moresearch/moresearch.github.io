...
The fix is the discipline the pattern language keeps naming: **the scope is a contract agreed before the work** — the [sprint contract](https://blog.hackspree.com/#harnessing-agentic-ai-systems-orchestrator-worker) that defines "done" before the agent starts, and the [tasks-that-fight-back](https://blog.hackspree.com/#agent-harnesses-need-tasks-that-fight-back) principle that keeps tasks small enough to score. Parkinson's Law is not defeated by asking the agent to be brief; it is defeated by making the container small.
